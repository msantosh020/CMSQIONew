...
public class ADFUtils {
    ...
    public static DCIteratorBinding findIterator(String pageDefName, String name) {
        ...
        DCIteratorBinding iter = bc.findIteratorBinding(name);
        if (iter == null) {
            throw new RuntimeException("Iterator '" + name + "' not found");
        }
        return iter;
    ...
    public static void queryOperationListenerWithReset(QueryOperationEvent queryOperationEvent, String vcQueryName, String resetObName, RichTable t1) {
        JSFUtils.resolveMethodExpression("#{bindings." + vcQueryName + ".processQueryOperation}", null, new Class[] { QueryOperationEvent.class }, new Object[] { queryOperationEvent });
        if (queryOperationEvent.getOperation().name().toUpperCase().equals("RESET")) {
            OperationBinding operationBinding = findOperationBinding(resetObName);
            operationBinding.execute();
            AdfFacesContext.getCurrentInstance().addPartialTarget(t1);
        }
    }

    public static void executeFilter(RichTable tbl, boolean clearFilter) {
        FilterableQueryDescriptor filterQD = (FilterableQueryDescriptor) tbl.getFilterModel();
        if (clearFilter && filterQD != null && filterQD.getFilterCriteria() != null) {
            filterQD.getFilterCriteria().clear();
        }
        tbl.queueEvent(new QueryEvent(tbl, filterQD));
        AdfFacesContext.getCurrentInstance().addPartialTarget(tbl);
    }

    public static boolean executeOperation(String oprName) {
        OperationBinding operationBinding = findOperationBinding(oprName);
    ...
    public static List<SelectItem> selectItemsForIteratorName(String iteratorName, String valueAttrName, String displayAttrName) {
    ...
    public static List<Long> attributeListForIteratorName(String iteratorName, String valueAttrName) {
        DCIteratorBinding iter = ADFUtils.findIterator(iteratorName);
        List<Long> attributeList = new ArrayList<Long>();
        for (Row r : iter.getAllRowsInRange()) {
            attributeList.add((Long) r.getAttribute(valueAttrName));
        }
        return attributeList;
    }
}
